...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (document.querySelector(".uniqueness__swiper")) {
  ...
  if (document.querySelector(".uniqueness-pro__swiper")) {
  ...
  */
  if (typeof gsap !== "undefined") {
    ...
    if (window.innerWidth > 1024) {
      ...
      if (verticalScroll.length > 0 && verticalScrollIn) {
      ...
      if (verticalScrollSecond.length > 0 && verticalScrollInSecond) {
        ScrollTrigger.create({
      ...
      let containerPro = document.querySelector(".uniqueness-pro__scroll");
      if (containerPro) {
        let slidesPro = document.querySelectorAll(".uniqueness-pro__scroll-item");
        let titlesPro = document.querySelectorAll(".uniqueness-pro__scroll-item-upper");
        let scrollWidthPro = containerPro.scrollWidth - containerPro.clientWidth;

        // Создаем кастомный скроллбар
        let scrollbarPro = document.createElement("div");
        scrollbarPro.classList.add("custom-scrollbar-pro");
        let scrollbarThumbPro = document.createElement("div");
        scrollbarThumbPro.classList.add("custom-scrollbar-thumb-pro");
        scrollbarPro.appendChild(scrollbarThumbPro);
        document.body.appendChild(scrollbarPro);

        // Рассчитываем ширину скроллбара
        let scrollbarWidthPro = window.innerWidth / (containerPro.scrollWidth / window.innerWidth);
        scrollbarThumbPro.style.width = `${scrollbarWidthPro}px`;
        gsap.set(scrollbarPro, {
          opacity: 0,
          pointerEvents: "none"
        });

        // Анимация горизонтального скролла
        let horizontalScrollPro = gsap.to(containerPro, {
          x: () => -scrollWidthPro,
          ease: "none",
          scrollTrigger: {
            trigger: containerPro,
            start: "top top",
            end: () => "+=" + scrollWidthPro,
            scrub: true,
            pin: true,
            ...
            onUpdate: self => {
              let progress = self.progress;
              gsap.to(scrollbarThumbPro, {
                x: progress * (window.innerWidth - scrollbarWidthPro),
                ease: "none"
              });

              // Определяем активный слайд по центру экрана
              let center = window.innerWidth / 2;
              let activeIndex = 0;
              let minDistance = Infinity;
              slidesPro.forEach((slide, index) => {
                let slideRect = slide.getBoundingClientRect();
                let slideCenter = slideRect.left + slideRect.width / 2;
                let distance = Math.abs(slideCenter - center);
                if (distance < minDistance) {
            ...
            onEnter: () => gsap.to(scrollbarPro, {
              opacity: 1,
              duration: 0.3,
              pointerEvents: "auto"
            }),
            onLeave: () => gsap.to(scrollbarPro, {
              opacity: 0,
              duration: 0.3,
              pointerEvents: "none"
            }),
            onEnterBack: () => gsap.to(scrollbarPro, {
              opacity: 1,
              duration: 0.3,
              pointerEvents: "auto"
            }),
            onLeaveBack: () => gsap.to(scrollbarPro, {
              opacity: 0,
              duration: 0.3,
              pointerEvents: "none"
            })
        ...
        let stylePro = document.createElement("style");
        stylePro.innerHTML = `
        ...
        `;
        document.head.appendChild(stylePro);
      }
    }
  }
});
